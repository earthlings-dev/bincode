...
where
  T: bincode::Encode + bincode::Decode<()> + serde::Serialize + serde::de::DeserializeOwned + core::fmt::Debug + PartialEq,
  C: bincode::config::Config + Copy,
{
  ...
  assert_eq!(compat_encoded, serde_encoded, "{t:?} encodes differently between the v1-compatible and serde APIs");
  ...
  let native_decoded: T = bincode::decode_from_slice(&compat_encoded, config).unwrap().0;
  assert_eq!(&native_decoded, t);

  // Deserialize via the serde API
  let serde_decoded: T = bincode::serde::decode_from_slice(&compat_encoded, config).unwrap().0;
  ...
  T: bincode::Encode + bincode::Decode<()> + serde::Serialize + serde::de::DeserializeOwned + core::fmt::Debug + PartialEq,
{
  ...
  test_same_with_config(&t, bincode::config::legacy().with_big_endian().with_variable_int_encoding());
  test_same_with_config(&t, bincode::config::legacy().with_little_endian().with_variable_int_encoding());
  test_same_with_config(&t, bincode::config::legacy().with_big_endian().with_fixed_int_encoding());
  test_same_with_config(&t, bincode::config::legacy().with_little_endian().with_fixed_int_encoding());
}
