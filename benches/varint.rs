...
fn slice_varint_u8(c: &mut Criterion) {
  ...
  let input: Vec<u8> = (0..10_000).map(|_| rng.random_range(0..u8::MAX)).collect();
...
fn slice_varint_u16(c: &mut Criterion) {
  let mut rng = rand::rng();
  let input: Vec<u16> = (0..10_000).map(|_| rng.random_range(0..u16::MAX)).collect();
  let config = config::standard();
  let bytes = bincode::encode_to_vec(input, config).unwrap();
...
fn slice_varint_u32(c: &mut Criterion) {
  let mut rng = rand::rng();
  let input: Vec<u32> = (0..10_000).map(|_| rng.random_range(0..u32::MAX)).collect();
  let config = config::standard();
  let bytes = bincode::encode_to_vec(input, config).unwrap();
...
fn slice_varint_u64(c: &mut Criterion) {
  let mut rng = rand::rng();
  let input: Vec<u64> = (0..10_000).map(|_| rng.random_range(0..u64::MAX)).collect();
  let config = config::standard();
  let bytes = bincode::encode_to_vec(input, config).unwrap();
...
fn bufreader_varint_u8(c: &mut Criterion) {
  ...
  c.bench_function("bufreader_varint_u8", |b| {
    b.iter(|| {
      let _: Vec<u8> = bincode::decode_from_reader(&mut std::io::BufReader::new(&bytes[..]), config).unwrap();
    })
  });
}
...
fn bufreader_varint_u16(c: &mut Criterion) {
  ...
  c.bench_function("bufreader_varint_u16", |b| {
    b.iter(|| {
      let _: Vec<u16> = bincode::decode_from_reader(&mut std::io::BufReader::new(&bytes[..]), config).unwrap();
    })
  });
}
...
fn bufreader_varint_u32(c: &mut Criterion) {
  ...
  c.bench_function("bufreader_varint_u32", |b| {
    b.iter(|| {
      let _: Vec<u32> = bincode::decode_from_reader(&mut std::io::BufReader::new(&bytes[..]), config).unwrap();
    })
  });
}
...
fn bufreader_varint_u64(c: &mut Criterion) {
  ...
  c.bench_function("bufreader_varint_u64", |b| {
    b.iter(|| {
      let _: Vec<u64> = bincode::decode_from_reader(&mut std::io::BufReader::new(&bytes[..]), config).unwrap();
    })
  });
}

criterion_group!(
  benches, slice_varint_u8, slice_varint_u16, slice_varint_u32, slice_varint_u64, bufreader_varint_u8, bufreader_varint_u16,
  bufreader_varint_u32, bufreader_varint_u64,
);
