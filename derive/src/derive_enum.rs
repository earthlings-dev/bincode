...
impl DeriveEnum {
            ...
            if let Some(fields) = variant.fields.as_ref() {
              ...
              match_body.group(delimiter, |field_body| {
                for (idx, field_name) in fields.names().into_iter().enumerate() {
                  ...
                  field_body.push(field_name.to_token_tree_with_prefix(TUPLE_FIELD_PREFIX));
                }
                Ok(())
              })?;
            }
            ...
            match_body.group(Delimiter::Brace, |body| {
              ...
              if let Some(fields) = variant.fields.as_ref() {
                for field_name in fields.names() {
                  let attributes = field_name.attributes().get_attribute::<FieldAttributes>()?.unwrap_or_default();
  ...
  fn invalid_variant_case(&self, enum_name: &str, result: &mut StreamBuilder) -> Result {
    ...
    result.group(Delimiter::Parenthesis, |err_inner| {
      err_inner.push_parsed(format!("{}::error::DecodeError::UnexpectedVariant", crate_name))?;
      err_inner.group(Delimiter::Brace, |variant_inner| {
        ...
        if self.variants.iter().any(|i| i.value.is_some()) {
          ...
          variant_inner.push_parsed(format!("&{}::error::AllowedEnumVariants::Allowed", crate_name))?;
    ...
    impl_for
      ...
      .body(|fn_builder| {
        if self.variants.is_empty() {
          ...
          fn_builder.push_parsed(format!("let variant_index = <u32 as {}::Decode::<__D::Context>>::decode(decoder)?;", crate_name))?;
                  ...
                  if let Some(fields) = variant.fields.as_ref() {
                    ...
                    for (idx, field) in fields.names().into_iter().enumerate() {
                      ...
                      if attributes.with_serde {
                        ...
                        variant_body.push_parsed(format!("{}::Decode::<__D::Context>::decode(decoder)?,", crate_name))?;
                      }
                    }
                  }
                  Ok(())
    ...
    impl_for
      .modify_generic_constraints(|generics, where_constraints| {
        if let Some((bounds, lit)) = (self.attributes.borrow_decode_bounds.as_ref()).or(self.attributes.bounds.as_ref()) {
          ...
          for g in generics.iter_generics() {
            where_constraints
              .push_constraint(g, format!("{}::de::BorrowDecode<'__de, {}>", crate_name, decode_context))
              .unwrap();
          }
      ...
      .with_generic_deps("__D", [format!(
        "{}::de::BorrowDecoder<'__de, Context = {}>",
        crate_name, decode_context
      )])
      .with_arg("decoder", "&mut __D")
      .with_return_type(format!("core::result::Result<Self, {}::error::DecodeError>", crate_name))
      .body(|fn_builder| {
                ...
                variant_case_body.group(Delimiter::Brace, |variant_body| {
                  if let Some(fields) = variant.fields.as_ref() {
                    ...
                    for (idx, field) in fields.names().into_iter().enumerate() {
                      ...
                      if attributes.with_serde {
                        variant_body.push_parsed(format!(
                          "<{0}::serde::BorrowCompat<_> as {0}::BorrowDecode::<__D::Context>>::borrow_decode(decoder)?.0,",
                          crate_name
                        ))?;
                      } else {
