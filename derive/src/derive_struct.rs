        ...
        if let Some(fields) = self.fields.as_ref() {
          for field in fields.names() {
            ...
            if attributes.with_serde {
              ...
              fn_body.push_parsed(format!("{}::Encode::encode(&self.{}, encoder)?;", crate_name, field))?;
            }
          }
        }
    ...
    impl_for
      ...
      .modify_generic_constraints(|generics, where_constraints| {
        if let Some((bounds, lit)) = (self.attributes.decode_bounds.as_ref()).or(self.attributes.bounds.as_ref()) {
          ...
          for g in generics.iter_generics() {
            where_constraints
              .push_constraint(g, format!("{}::Decode<{}>", crate_name, decode_context))
              .unwrap();
          }
        }
        Ok(())
              ...
              for field in fields.names() {
                ...
                if attributes.with_serde {
                  struct_body.push_parsed(format!(
                    ...
                    crate_name, field, decode_context,
                  ))?;
                } else {
                  struct_body.push_parsed(format!("{1}: {0}::Decode::decode(decoder)?,", crate_name, field))?;
                }
              }
            }
            Ok(())
