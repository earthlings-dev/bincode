...
///
/// Some types may require specific contexts. For example, to decode arena-based collections, an
/// arena allocator must be provided as a context. In these cases, the context type `Context` should
/// be specified or bounded.
///
/// This trait should be implemented for types which do not have references to data in the reader.
/// For types that contain e.g. `&str` and `&[u8]`, implement [BorrowDecode] instead.
///
/// Whenever you derive `Decode` for your type, the base trait `BorrowDecode` is automatically
/// implemented.
///
/// This trait will be automatically implemented with unbounded `Context` if you enable the `derive`
/// feature and add `#[derive(bincode::Decode)]` to your type. Note that if the type contains any
/// lifetimes, `BorrowDecode` will be implemented instead.
///
/// # Implementing this trait manually
///
/// If you want to implement this trait for your type, the easiest way is to add a
/// `#[derive(bincode::Decode)]`, build and check your `target/generated/bincode/` folder. This
/// should generate a `<Struct name>_Decode.rs` file.
///
/// For this struct:
///
/// ```
...
///     Ok(Self {
///       x: bincode::Decode::decode(decoder)?, y: bincode::Decode::decode(decoder)?
///     })
///   }
/// }
...
///
/// This trait should be implemented for types that contain borrowed data, like `&str` and `&[u8]`.
/// If your type does not have borrowed data, consider implementing [Decode] instead.
///
/// This trait will be automatically implemented if you enable the `derive` feature and add
/// `#[derive(bincode::Decode)]` to a type with a lifetime.
...
pub trait Decoder: Sealed {
  ...
  ///
  /// When decoding container types, a typical implementation would claim to read `len *
  /// size_of::<T>()` bytes. This is to ensure that bincode won't allocate several GB of memory
  /// while constructing the container.
  ///
  /// Because the implementation claims `len * size_of::<T>()`, but then has to decode each `T`,
  /// this would be marked as double. This function allows us to un-claim each `T` that gets
  /// decoded.
  ///
  /// We cannot check if `len * size_of::<T>()` is valid without claiming it, because this would
  /// mean that if you have a nested container (e.g. `Vec<Vec<T>>`), it does not know how much
  /// memory is already claimed, and could easily allocate much more than the user intends.
  /// ```
...
#[inline]
pub(crate) fn decode_option_variant<D: Decoder>(decoder: &mut D, type_name: &'static str) -> Result<Option<()>, DecodeError> {
