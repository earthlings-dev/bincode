...
//!
//! *Important* make sure you use the same config for encoding and decoding, or else bincode will
//! not work properly.
//!
//! To use a config, first create a type of [Configuration]. This type will implement trait [Config]
//! for use with bincode.
//!
//! ```
...
/// The Configuration struct is used to build bincode configurations. The [Config] trait is
/// implemented by this struct when a valid configuration has been constructed.
///
/// The following methods are mutually exclusive and will overwrite each other. The last call to one
/// of these methods determines the behavior of the configuration:
///
...
const fn generate<E, I, L, T>() -> Configuration<E, I, L, T> {
  Configuration {
    _e: PhantomData, _i: PhantomData, _l: PhantomData, _t: PhantomData
  }
}
...
impl<E, I, L, T> Configuration<E, I, L, T> {
  ...
  /// 5. If `2**64 <= u < 2**128`, encode it as a literal byte 254, followed by a u128 with value
  ///    `u`.
  ///
...
pub trait Config: InternalEndianConfig + InternalIntEncodingConfig + InternalLimitConfig + InternalTrailingConfig + Copy + Clone {
...
impl<C> Config for C
where
  C: InternalEndianConfig + InternalIntEncodingConfig + InternalLimitConfig + InternalTrailingConfig + Copy + Clone,
{
  fn endianness(&self) -> Endianness {
...
mod internal {
  ...
  impl<E, I: InternalIntEncodingConfig, L, T> InternalIntEncodingConfig for Configuration<E, I, L, T> {
