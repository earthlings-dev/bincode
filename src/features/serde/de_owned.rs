...
impl<DE: Decoder> OwnedSerdeDecoder<DE> {
  ...
  pub fn as_deserializer<'a>(&'a mut self) -> impl for<'de> serde::Deserializer<'de, Error = DecodeError> + 'a {
    SerdeDecoder {
      de: &mut self.de
    }
  }
...
impl<'r, C: Config, R: std::io::Read> OwnedSerdeDecoder<DecoderImpl<IoReader<&'r mut R>, C, ()>> {
  ...
  pub fn from_std_read(src: &'r mut R, config: C) -> OwnedSerdeDecoder<DecoderImpl<IoReader<&'r mut R>, C, ()>>
  where
    C: Config,
  {
...
/// Decode type `D` from the given reader with the given `Config`. The reader can be any type that
/// implements `std::io::Read`, e.g. `std::fs::File`.
///
...
pub fn decode_from_std_read<'r, D: DeserializeOwned, C: Config, R: std::io::Read>(src: &'r mut R, config: C) -> Result<D, DecodeError> {
  let mut serde_decoder = OwnedSerdeDecoder::<DecoderImpl<IoReader<&'r mut R>, C, ()>>::from_std_read(src, config);
...
#[cfg(feature = "std")]
#[cfg_attr(docsrs, doc(cfg(feature = "std")))]
pub fn seed_decode_from_std_read<'de, 'r, D, C, R>(seed: D, src: &'r mut R, config: C) -> Result<D::Value, DecodeError>
where
...
///
/// [config]: ../config/index.html
pub fn decode_from_reader<D: DeserializeOwned, R: Reader, C: Config>(reader: R, config: C) -> Result<D, DecodeError> {
...
pub fn seed_decode_from_reader<'de, D, R, C>(seed: D, reader: R, config: C) -> Result<D::Value, DecodeError>
where
  D: DeserializeSeed<'de>,
  R: Reader,
  C: Config,
...
impl<'de, DE: Decoder> Deserializer<'de> for SerdeDecoder<'_, DE> {
  ...
  {
    Err(crate::error::DecodeError::Serde(SerdeDecodeError::AnyNotSupported))
  }
  ...
  where
    V: serde::de::Visitor<'de>,
  {
    Err(crate::error::DecodeError::Serde(SerdeDecodeError::CannotBorrowOwnedData))
  }
  ...
  where
    V: serde::de::Visitor<'de>,
  {
    Err(crate::error::DecodeError::Serde(SerdeDecodeError::CannotAllocate))
  }
    ...
    V: serde::de::Visitor<'de>,
  {
    ...
    impl<'de, 'a, 'b: 'a, DE: Decoder + 'b> SeqAccess<'de> for Access<'a, 'b, DE> {
        ...
        T: DeserializeSeed<'de>,
      {
        if self.len > 0 {
          self.len -= 1;
          let value = DeserializeSeed::deserialize(seed, SerdeDecoder {
    ...
    V: serde::de::Visitor<'de>,
  {
    ...
    impl<'de, 'a, 'b: 'a, DE: Decoder + 'b> MapAccess<'de> for Access<'a, 'b, DE> {
        ...
        K: DeserializeSeed<'de>,
      {
        if self.len > 0 {
          self.len -= 1;
          let key = DeserializeSeed::deserialize(seed, SerdeDecoder {
            de: self.deserializer.de
          })?;
          Ok(Some(key))
        } else {
  ...
  where
    V: serde::de::Visitor<'de>,
  {
    Err(crate::error::DecodeError::Serde(SerdeDecodeError::IdentifierNotSupported))
  }
  ...
  where
    V: serde::de::Visitor<'de>,
  {
    Err(crate::error::DecodeError::Serde(SerdeDecodeError::IgnoredAnyNotSupported))
  }
