...
///
/// Trailing bytes are allowed (matching v1 behavior).
pub fn deserialize_in_place<'a, T: ::serde::Deserialize<'a>>(
    bytes: &'a [u8],
    place: &mut T,
) -> Result<(), DecodeError> {
    let mut serde_decoder =
        super::de_borrowed::BorrowedSerdeDecoder::from_slice(bytes, crate::config::legacy(), ());
...
/// Use [`options()`] to get a `DefaultOptions` value, or [`config::legacy()`](crate::config::legacy)
/// for the v1-default configuration (fixint + little-endian).
///
/// # Trailing bytes
///
...
pub trait Options: Config + Sized {
    ...
    /// If this config rejects trailing bytes, an error is returned when not all
    /// bytes in the slice are consumed.
    fn deserialize_in_place<'a, T: ::serde::Deserialize<'a>>(
        &self,
        bytes: &'a [u8],
        place: &mut T,
    ) -> Result<(), DecodeError> {
        let mut serde_decoder =
            super::de_borrowed::BorrowedSerdeDecoder::from_slice(bytes, *self, ());
        ::serde::Deserialize::deserialize_in_place(serde_decoder.as_deserializer(), place)?;
