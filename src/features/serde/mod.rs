...
//! - [BorrowCompat]
//!
//! For interop with bincode's `derive` feature, you can use the `#[bincode(with_serde)]` attribute
//! on each field that implements serde's traits.
//!
//! ```
//! # #[cfg(feature = "derive")]
//! # mod foo {
...
//! # Known issues
//!
//! Because bincode is a format without meta data, there are several known issues with serde's
//! attributes. Please do not use any of the following attributes if you plan on using bincode, or
//! use bincode's own `derive` macros.
//! - `#[serde(flatten)]`
//! - `#[serde(skip)]`
...
//! - `#[serde(untagged)]`
//!
//! **Using any of the above attributes can and will cause issues with bincode and will result in
//! lost data**. Consider using bincode's own derive macro instead.
//!
...
//! - Serde has chosen to not have a MSRV ([source](https://github.com/serde-rs/serde/pull/2257)).
//!   We think MSRV is important, bincode 1 still compiles with rust 1.18.
//! - Before serde we had rustc-serializer. Serde has more than replaced rustc-serializer, but we
//!   can imagine a future where serde is replaced by something else.
//! - We believe that fewer dependencies are better and that you should be able to choose your own
//!   dependencies. With all features disabled, bincode has one direct dependency: [`unty`], a micro
//!   crate we manage ourselves.
//!
//! **note:** just because we're making serde an optional dependency, it does not mean we're
//! dropping support for serde. Serde will still be fully supported, we're just giving you the
...
pub use self::compat::DefaultOptions;
pub use self::compat::Options;
pub use self::compat::deserialize;
#[cfg(feature = "std")]
pub use self::compat::deserialize_from;
pub use self::compat::deserialize_from_reader;
pub use self::compat::deserialize_in_place;
pub use self::compat::options;
#[cfg(feature = "alloc")]
pub use self::compat::serialize;
#[cfg(feature = "std")]
pub use self::compat::serialize_into;
pub use self::compat::serialized_size;
...
pub enum DecodeError {
  ...
  /// Custom serde error but bincode is unable to allocate a string. Set a breakpoint where this is
  /// thrown for more information.
  #[cfg(not(feature = "alloc"))]
  CustomError,
}
...
/// Wrapper struct that implements [Decode] and [Encode] on any type that implements serde's
/// [DeserializeOwned] and [Serialize] respectively.
///
/// This works for most types, but if you're dealing with borrowed data consider using
/// [BorrowCompat] instead.
///
/// [Decode]: ../de/trait.Decode.html
/// [Encode]: ../enc/trait.Encode.html
...
where
  T: serde::de::DeserializeOwned,
{
  fn borrow_decode<D: crate::de::BorrowDecoder<'de>>(decoder: &mut D) -> Result<Self, crate::error::DecodeError> {
    let serde_decoder = de_owned::SerdeDecoder {
      de: decoder
    };
    T::deserialize(serde_decoder).map(Compat)
  ...
  T: serde::Serialize,
{
  fn encode<E: crate::enc::Encoder>(&self, encoder: &mut E) -> Result<(), crate::error::EncodeError> {
    let serializer = ser::SerdeEncoder {
      enc: encoder
    };
...
/// Wrapper struct that implements [BorrowDecode] and [Encode] on any type that implements serde's
/// [Deserialize] and [Serialize] respectively. This is mostly used on `&[u8]` and `&str`, for other
/// types consider using [Compat] instead.
///
  ...
  T: serde::de::Deserialize<'de>,
{
  fn borrow_decode<D: crate::de::BorrowDecoder<'de>>(decoder: &mut D) -> Result<Self, crate::error::DecodeError> {
    let serde_decoder = de_borrowed::SerdeDecoder {
      de: decoder, pd: core::marker::PhantomData
    };
