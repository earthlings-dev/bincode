...
impl<'de, DE: BorrowDecoder<'de>> BorrowedSerdeDecoder<'de, DE> {
  ...
  pub fn as_deserializer<'a>(&'a mut self) -> impl serde::Deserializer<'de, Error = DecodeError> + 'a {
    SerdeDecoder {
      de: &mut self.de, pd: PhantomData
    }
  }
}
...
impl<'de, C: Config, Context> BorrowedSerdeDecoder<'de, DecoderImpl<SliceReader<'de>, C, Context>> {
  ...
  pub fn from_slice(slice: &'de [u8], config: C, context: Context) -> BorrowedSerdeDecoder<'de, DecoderImpl<SliceReader<'de>, C, Context>>
  where
    C: Config,
  {
...
pub fn borrow_decode_from_slice<'de, D, C>(slice: &'de [u8], config: C) -> Result<(D, usize), DecodeError>
where
  D: Deserialize<'de>,
  C: Config,
{
  let mut serde_decoder = BorrowedSerdeDecoder::<DecoderImpl<SliceReader<'de>, C, ()>>::from_slice(slice, config, ());
...
/// Decode a borrowed type from the given slice using a seed. Some parts of the decoded type are
/// expected to be referring to the given slice
pub fn seed_decode_from_slice<'de, D, C>(seed: D, slice: &'de [u8], config: C) -> Result<(D::Value, usize), DecodeError>
where
        ...
        T: DeserializeSeed<'de>,
      {
        if self.len > 0 {
          ...
          let value = DeserializeSeed::deserialize(seed, SerdeDecoder {
            de: self.deserializer.de, pd: PhantomData
          })?;
          Ok(Some(value))
        } else {
          Ok(None)
