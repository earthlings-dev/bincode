...
/// Encode the given value into a `Vec<u8>` with the given `Config`. See the [config] module for
/// more information.
///
...
where
  ...
  C: Config,
{
  let mut encoder = crate::enc::EncoderImpl::new(crate::enc::write::SliceWriter::new(dst), config);
  let serializer = SerdeEncoder {
    enc: &mut encoder
  };
  val.serialize(serializer)?;
...
///
/// [config]: ../config/index.html
pub fn encode_into_writer<E: Serialize, W: Writer, C: Config>(val: E, writer: W, config: C) -> Result<(), EncodeError> {
...
#[cfg(feature = "std")]
pub fn encode_into_std_write<E: Serialize, C: Config, W: std::io::Write>(val: E, dst: &mut W, config: C) -> Result<usize, EncodeError> {
  ...
  ENC: Encoder,
{
  ...
  fn serialize_seq(mut self, len: Option<usize>) -> Result<Self::SerializeSeq, Self::Error> {
    let len = len.ok_or(crate::error::EncodeError::Serde(SerdeEncodeError::SequenceMustHaveLength))?;
  ...
  {
    Err(crate::error::EncodeError::Serde(SerdeEncodeError::CannotCollectStr))
  }
