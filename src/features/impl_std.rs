...
pub fn decode_from_std_read<D: Decode<()>, C: Config, R: std::io::Read>(src: &mut R, config: C) -> Result<D, DecodeError> {
  decode_from_std_read_with_context(src, config, ())
}

/// Decode type `D` from the given reader with the given `Config` and `Context`. The reader can be
/// any type that implements `std::io::Read`, e.g. `std::fs::File`.
///
/// See the [config] module for more information about config options.
///
/// [config]: config/index.html
  ...
  R: std::io::Read,
{
  ...
  fn read(&mut self, bytes: &mut [u8]) -> Result<(), DecodeError> {
    self.reader.read_exact(bytes).map_err(|inner| DecodeError::Io {
      inner,
...
/// with the given `Config`. See the [config] module for more information.
/// Returns the amount of bytes written.
///
/// [config]: config/index.html
#[cfg_attr(docsrs, doc(cfg(feature = "std")))]
pub fn encode_into_std_write<E: Encode, C: Config, W: std::io::Write>(val: E, dst: &mut W, config: C) -> Result<usize, EncodeError> {
...
impl<W: std::io::Write> Writer for IoWriter<'_, W> {
  ...
  fn write(&mut self, bytes: &[u8]) -> Result<(), EncodeError> {
    self.writer.write_all(bytes).map_err(|inner| EncodeError::Io {
      inner,
  ...
  fn encode<E: Encoder>(&self, encoder: &mut E) -> Result<(), EncodeError> {
    let duration = self
      ...
      .map_err(|e| EncodeError::InvalidSystemTime {
        inner: e, time: std::boxed::Box::new(*self)
      })?;
...
impl<Context> Decode<Context> for SystemTime {
  fn decode<D: Decoder<Context = Context>>(decoder: &mut D) -> Result<Self, DecodeError> {
    ...
    match SystemTime::UNIX_EPOCH.checked_add(duration) {
      Some(t) => Ok(t),
      None => Err(DecodeError::InvalidSystemTime {
        duration,
      }),
    }
  }
