...
/// Trait that indicates that a struct can be used as a destination to encode data too. This is used
/// by [Encode]
///
/// [Encode]: ../trait.Encode.html
pub trait Writer {
  /// Write `bytes` to the underlying writer. Exactly `bytes.len()` bytes must be written, or else
...
impl<'storage> SliceWriter<'storage> {
  ...
  pub fn new(bytes: &'storage mut [u8]) -> SliceWriter<'storage> {
    ...
    SliceWriter {
      slice: bytes, original_length: original
    }
  }
...
/// A writer that counts how many bytes were written. This is useful for e.g. pre-allocating buffers
/// bfeore writing to them.
#[derive(Default)]
