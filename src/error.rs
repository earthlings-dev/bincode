...
pub enum DecodeError {
  ...
  UnexpectedEnd {
    ...
    ///
    /// **Note**: Bincode has no look-ahead mechanism. This means that this will only return the
    /// amount of bytes to be read for the current action, and not take into account the entire data
  ...
  /// Invalid type was found. The decoder tried to read type `expected`, but found type `found`
  /// instead.
  InvalidIntegerType {
  ...
  /// Invalid enum variant was found. The decoder tried to decode variant index `found`, but the
  /// variant index should be between `min` and `max`.
  UnexpectedVariant {
  ...
  /// The decoder tried to decode a `char` and failed. The given buffer contains the bytes that are
  ...
  /// The decoder tried to decode an array of length `required`, but the binary data contained an
  /// array of length `found`.
  ArrayLengthMismatch {
  ...
  pub(crate) fn change_integer_type_to_signed(self) -> DecodeError {
    match self {
      Self::InvalidIntegerType {
        ...
        expected: expected.into_signed(), found: found.into_signed()
      },
      other => other,
    }
  }
...
/// Integer types. Used by [DecodeError]. These types have no purpose other than being shown in
/// errors.
#[non_exhaustive]
#[derive(Debug, PartialEq, Eq)]
#[allow(missing_docs)]
