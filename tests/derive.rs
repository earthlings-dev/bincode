...
fn test_encode() {
  let start = Test {
    a: 5i32, b: 10u32, c: 20u8
  };
  let mut slice = [0u8; 1024];
  let bytes_written = bincode::encode_into_slice(start, &mut slice, bincode::config::standard()).unwrap();
...
{
  fn decode<D: ::bincode::de::Decoder<Context = Context>>(decoder: &mut D) -> core::result::Result<Self, ::bincode::error::DecodeError> {
    Ok(Self {
      a: ::bincode::Decode::decode(decoder)?, b: ::bincode::Decode::decode(decoder)?, c: ::bincode::Decode::decode(decoder)?
    })
  }
}
...
fn test_decode() {
  let start = Test2 {
    a: 5u32, b: 10u32, c: 1024u32
  };
  let slice = [5, 10, 251, 0, 4];
  let (result, len): (Test2<u32>, usize) = bincode::decode_from_slice(&slice, bincode::config::standard()).unwrap();
...
fn test_encode_decode_str() {
  let start = Test3 {
    a: "Foo bar", b: 10u32, c: 1024u32, d: Some(b"Foo bar")
  };
  let mut slice = [0u8; 100];

  let len = bincode::encode_into_slice(&start, &mut slice, bincode::config::standard()).unwrap();
  assert_eq!(len, 21);
  let (end, len): (Test3, usize) = bincode::borrow_decode_from_slice(&slice[..len], bincode::config::standard()).unwrap();
  assert_eq!(end, start);
  assert_eq!(len, 21);
}
...
fn test_decode_tuple() {
  let start = TestTupleStruct(5, 10, 1024);
  let slice = [5, 10, 251, 0, 4];
  let (result, len): (TestTupleStruct, usize) = bincode::decode_from_slice(&slice, bincode::config::standard()).unwrap();
  assert_eq!(result, start);
  assert_eq!(len, 5);
}
...
fn test_decode_enum_struct_variant() {
  ...
  let slice = [1, 5];
  let (result, len): (TestEnum, usize) = bincode::decode_from_slice(&slice, bincode::config::standard()).unwrap();
  assert_eq!(result, start);
  assert_eq!(len, 2);
}
...
fn test_decode_borrowed_enum_struct_variant() {
  ...
  let (result, len): (TestEnum2, usize) = bincode::borrow_decode_from_slice(&slice, bincode::config::standard()).unwrap();
  assert_eq!(result, start);
  assert_eq!(len, 5);
}
...
fn test_c_style_enum() {
  fn ser(e: CStyleEnum) -> u8 {
    ...
    let bytes_written = bincode::encode_into_slice(e, &mut slice, bincode::config::standard()).unwrap();
    assert_eq!(bytes_written, 1);
    slice[0]
  }
...
fn test_macro_newtype() {
  for val in [0, 100, usize::MAX] {
    ...
    let usize_len = bincode::encode_into_slice(val, &mut usize_slice, bincode::config::standard()).unwrap();

    let mut newtype_slice = [0u8; 10];
    let newtype_len = bincode::encode_into_slice(MacroNewType(val), &mut newtype_slice, bincode::config::standard()).unwrap();

    assert_eq!(usize_len, newtype_len);
    assert_eq!(usize_slice, newtype_slice);

    let (newtype, len) = bincode::decode_from_slice::<MacroNewType, _>(&newtype_slice, bincode::config::standard()).unwrap();
...
fn test_enum_with_generics_roundtrip() {
  ...
  let decoded: TestWithGeneric<u32> = bincode::decode_from_slice(&slice[..bytes_written], bincode::config::standard())
    .unwrap()
    .0;
  ...
  let decoded: TestWithGeneric<()> = bincode::decode_from_slice(&slice[..bytes_written], bincode::config::standard())
    .unwrap()
    .0;
  assert_eq!(start, decoded);
}
...
mod zoxide {
  ...
  fn test() {
    let dirs = vec![
      Dir {
        path: Cow::Borrowed("Foo"), rank: 1.23, last_accessed: 5
      },
      Dir {
        path: Cow::Owned(String::from("Bar")), rank: 2.34, last_accessed: 10
      },
    ];
    let config = bincode::config::standard();
