...
fn test_serde_round_trip() {
  ...
  let bytes = bincode::serde::encode_to_vec(
    SerdeRoundtrip {
      a: 15, b: 15, c: TupleS(2.0, 3.0, 4.0)
    },
    bincode::config::standard(),
  )
  .unwrap();
  assert_eq!(bytes, &[15, 0, 0, 0, 64, 0, 0, 64, 64, 0, 0, 128, 64]);
  let (result, len): (SerdeRoundtrip, usize) = bincode::serde::decode_from_slice(&bytes, bincode::config::standard()).unwrap();
...
fn test_serialize_deserialize_borrowed_data() {
  ...
  let len = bincode::serde::encode_into_slice(&input, &mut result, bincode::config::standard()).unwrap();
...
fn test_serialize_deserialize_owned_data() {
  let input = SerdeWithOwnedData {
    a: 5, b: 5, str: String::from("Hello world")
  };
  ...
  let (output, len): (SerdeWithOwnedData, usize) = bincode::serde::decode_from_slice(&result, bincode::config::standard()).unwrap();
  assert_eq!(
    SerdeWithOwnedData {
      b: 0, // remember: b is skipped
      ..input
...
mod derive {
  ...
  fn test_serde_derive() {
      ...
      T: bincode::Encode + bincode::Decode<()> + PartialEq + core::fmt::Debug,
    {
      ...
      let slice = &slice[..len];
      let (result, len): (T, usize) = bincode::decode_from_slice(slice, bincode::config::standard()).unwrap();
