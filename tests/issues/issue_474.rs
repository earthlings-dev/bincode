...
impl MemCache {
  fn set_data<T>(&self, key: &Uuid, cache_data: &T, expire_seconds: i64) -> Result<(), bincode::error::EncodeError>
  where
    ...
    T: Send + Sync + DeserializeOwned,
  {
    ...
    let (decoded, _len): (T, usize) = bincode::serde::decode_from_slice(&cache_item.payload[..], config)?;
    Ok(decoded)
  }
}
