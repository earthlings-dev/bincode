...
fn test_refcell_already_borrowed() {
  ...
  let result =
    bincode::encode_into_slice(&cell, &mut slice, bincode::config::standard()).expect_err("Encoding a borrowed refcell should fail");

  match result {
    bincode::error::EncodeError::RefCellAlreadyBorrowed {
      ..
    } => {} // ok
...
fn test_slice() {
  ...
  let (output, len): (&[u8], usize) = bincode::borrow_decode_from_slice(&buffer[..8], bincode::config::standard()).unwrap();
...
fn test_option_slice() {
  ...
  let (output, len): (Option<&[u8]>, usize) = bincode::borrow_decode_from_slice(&buffer[..n], bincode::config::standard()).unwrap();
  assert_eq!(input, output);
  assert_eq!(len, n);
...
fn test_str() {
  ...
  bincode::encode_into_slice(input, &mut buffer, bincode::config::standard()).unwrap();
  assert_eq!(&buffer[..12], &[11, 72, 101, 108, 108, 111, 32, 119, 111, 114, 108, 100]);

  let (output, len): (&str, usize) = bincode::borrow_decode_from_slice(&buffer[..12], bincode::config::standard()).unwrap();
  assert_eq!(input, output);
  assert_eq!(len, 12);
}
...
fn test_option_str() {
  ...
  let n = bincode::encode_into_slice(input, &mut buffer, bincode::config::standard()).unwrap();
  assert_eq!(&buffer[..n], &[1, 11, 72, 101, 108, 108, 111, 32, 119, 111, 114, 108, 100]);

  let (output, len): (Option<&str>, usize) = bincode::borrow_decode_from_slice(&buffer[..n], bincode::config::standard()).unwrap();
  assert_eq!(input, output);
  assert_eq!(len, n);
...
fn test_array() {
  ...
  let (output, len): ([u8; 10], usize) = bincode::decode_from_slice(&buffer[..11], bincode::config::standard()).unwrap();
  assert_eq!(input, output);
  assert_eq!(len, 10);
...
fn test_duration_out_of_range() {
  ...
  bincode::encode_into_slice((u64::MAX, u32::MAX), &mut input, bincode::config::standard()).unwrap();

  let result: Result<(std::time::Duration, usize), _> = bincode::decode_from_slice(&input, bincode::config::standard());
...
fn test_duration_wrapping() {
  ...
  bincode::encode_into_slice((u64::MAX - 4, u32::MAX), &mut input, bincode::config::standard()).unwrap();

  let (result, _): (std::time::Duration, _) = bincode::decode_from_slice(&input, bincode::config::standard()).unwrap();
