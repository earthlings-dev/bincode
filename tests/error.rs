...
fn core_error_sources_are_preserved() {
    ...
    assert!(
        error
            .source()
            .unwrap()
            .downcast_ref::<core::cell::BorrowError>()
            .is_some()
    );
    drop(borrowed);
    ...
    let error = DecodeError::Utf8 { inner };
    assert!(
        error
            .source()
            .unwrap()
...
fn std_error_sources_are_preserved_through_core_error() {
    let error = EncodeError::Io {
        inner: std::io::Error::other("io failure"),
        index: 0,
    };
    assert!(
        error
    ...
    let error = EncodeError::InvalidSystemTime {
        inner: before_epoch,
        time: Box::new(std::time::UNIX_EPOCH),
    };
    assert!(
        error
            .source()
    ...
    let error = DecodeError::Io {
        inner: std::io::Error::other("opaque decode I/O failure"),
        additional: 1,
    };
    assert!(error.source().is_none());
}
