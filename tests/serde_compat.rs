...
fn options_serialize_equals_encode_to_vec() {
    let val = sample();
    let options_bytes = bincode::options()
...
fn trailing_bytes_reject() {
    ...
    let result: Result<u8, _> = bincode::options()
        .with_fixed_int_encoding()
        .deserialize(&bytes);
...
fn deserialize_from_reader_via_options() {
    ...
    let bytes = bincode::options()
        ...
        .unwrap();
    let reader = bincode::de::read::SliceReader::new(&bytes);
    let decoded: TestStruct = bincode::options()
        .with_big_endian()
