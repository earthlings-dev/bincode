...
impl<'bump, T: Decode<&'bump Bump>> Decode<&'bump Bump> for CodableVec<'bump, T> {
  fn decode<D: bincode::de::Decoder<Context = &'bump Bump>>(decoder: &mut D) -> Result<Self, bincode::error::DecodeError> {
...
impl<'de, 'bump, T: BorrowDecode<'de, &'bump Bump>> BorrowDecode<'de, &'bump Bump> for CodableVec<'bump, T> {
  fn borrow_decode<D: BorrowDecoder<'de, Context = &'bump Bump>>(decoder: &mut D) -> Result<Self, DecodeError> {
    let len = u64::decode(decoder)?;
    let len = usize::try_from(len).map_err(|_| DecodeError::OutsideUsizeRange(len))?;
...
impl<Context> Decode<Context> for SelfReferencing {
  fn decode<D: bincode::de::Decoder<Context = Context>>(decoder: &mut D) -> Result<Self, DecodeError> {
    SelfReferencing::try_new(Bump::new(), |bump| Container::decode(&mut decoder.with_context(bump)))
  }
}
...
fn decode_with_context() {
  ...
  let (decoded_container, _) = decode_from_slice_with_context::<_, Container, _>(&bytes, config, &bump).unwrap();
