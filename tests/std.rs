...
impl<Context> bincode::Decode<Context> for Foo {
  fn decode<D: bincode::de::Decoder>(decoder: &mut D) -> Result<Self, bincode::error::DecodeError> {
    Ok(Self {
      a: bincode::Decode::decode(decoder)?, b: bincode::Decode::decode(decoder)?
    })
  }
}
...
fn test_std_commons() {
  ...
  the_same(SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::LOCALHOST, 12345)));
  the_same(SocketAddr::V6(SocketAddrV6::new(Ipv6Addr::LOCALHOST, 12345, 0, 0)));
  the_same_with_comparer(Mutex::new("Hello world".to_string()), |a, b| *a.lock().unwrap() == *b.lock().unwrap());
  the_same_with_comparer(RwLock::new("Hello world".to_string()), |a, b| *a.read().unwrap() == *b.read().unwrap());
  ...
  let mut custom_map: std::collections::HashMap<String, String, MyBuildHasher> = Default::default();
  ...
  let (decoded, len): (CString, usize) = bincode::decode_from_slice(&buffer[..len], config).unwrap();
  ...
  let (decoded, len): (&Path, usize) = bincode::borrow_decode_from_slice(&buffer[..len], config).unwrap();
...
fn test_system_time_out_of_range() {
  ...
  let result: Result<(std::time::SystemTime, usize), _> = bincode::decode_from_slice(&input, bincode::config::standard());
  ...
  match result {
    ...
    Err(DecodeError::InvalidSystemTime {
      ...
      assert_eq!(duration, std::time::Duration::new(10447520527445462160, 144))
    }
    Err(e) => panic!("Expected DecodeError::InvalidSystemTime, got {e:?}"),
  }
}

/// Simple example of user-defined hasher to test encoding/decoding HashMap and HashSet with custom
/// hash algorithms.
...
fn test_decode_borrow_str_in_array() {
  let (strs, _): (Vec<&str>, usize) =
    bincode::borrow_decode_from_slice(&[3, 3, b'a', b'b', b'c', 3, b'd', b'e', b'f', 3, b'g', b'h', b'i'], bincode::config::standard())
      .unwrap();
  assert_eq!(strs, vec!["abc", "def", "ghi"]);

  let (strs, _): ([&str; 3], usize) =
    bincode::borrow_decode_from_slice(&[3, b'a', b'b', b'c', 3, b'd', b'e', b'f', 3, b'g', b'h', b'i'], bincode::config::standard())
      .unwrap();
